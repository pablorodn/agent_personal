...
## Fase 14 - Cierre y hardening (intento, sin cerrar como HECHO)

- Fecha: 2026-07-05.
- Baseline de fase (previo a cambios de código): `ruff check .` OK, `mypy app/` OK (55 archivos), `pytest -q` OK (`115 passed`) — coincide con el "Final de fase" reportado al cierre de Fase 13, sobre working tree limpio en el commit `cfad018`.
- Investigación previa a preguntar (punto 4 del checklist de preguntas, resuelta sin necesidad de pregunta): se identificaron los 3 `type: ignore[call-arg]` existentes (`app/config.py:50`, `app/agent/model.py:50` y `:61`) y se removió cada uno temporalmente para verificar con `mypy app/` si generaban error real. **Los 3 son necesarios, ninguno es unused**: sin ellos, mypy reporta 11 errores reales (`Settings()` requiere los kwargs con alias de `pydantic-settings`; `ChatOpenAI(...)` no tiene `max_tokens`/`openai_api_key`/`openai_api_base` en sus stubs de `langchain-openai`). El checklist original ("limpiar los 3 unused type: ignore reportados por mypy") estaba basado en un estado desactualizado del proyecto (probablemente de una versión anterior de las dependencias, ver menciones de "unused-ignore" en el baseline técnico de Fase 2); con las versiones actualmente instaladas, `mypy app/` con `warn_unused_ignores = true` (ya activo en `pyproject.toml`) no reporta ninguno de los 3 como unused. Se dejaron los 3 sin cambios; el ítem del checklist se marca como cumplido en el sentido de "investigado y confirmado que no hay unused ignores que limpiar", no como código modificado.
- Valores no fijados por el checklist, confirmados explícitamente por el usuario antes de implementar:
  - `secure=True` condicional por entorno: se agregó variable nueva `ENVIRONMENT` (`Settings.environment`, default `"development"`, alias `ENVIRONMENT`) con propiedad `Settings.is_production` (`environment == "production"`); cualquier valor distinto de `"production"` (incluida ausencia de la variable) resuelve a `secure=False`/`https_only=False`, preservando el comportamiento actual en desarrollo/tests sin cambios.
  - Columnas de `agent_sessions` ausentes del modelo Pydantic: se agregaron las 4 al modelo `AgentSession` (`budget_tokens_used: int`, `budget_tokens_limit: int`, `created_at: str`, `updated_at: str`) en vez de documentar la omisión, aunque el código de la aplicación no las use todavía.
  - Dónde documentar `POST /api/chat/stream`: resuelto sin necesidad de pregunta al usuario — el propio `docs/technical-brief.md` §4 ya tenía el placeholder "Documentar formalmente `POST /api/chat/stream` en fase de cierre/hardening", y el checklist dice literalmente "en brief y UI"; se documentó en ambos (`docs/technical-brief.md` §4 y `docs/ui-design.md` §5/§6/§11).
- Hallazgo Fase 9/11 (bug de `chat.html`), investigado y confirmado real: en `submitChat()`, el orden original era `renderOutgoingMessage(form)` (que ejecuta `input.value = ""` sobre `#chat-input` para dar feedback visual inmediato) **seguido de** `const formData = new FormData(form)`. Por spec de `FormData`, el constructor lee el valor *actual* (en vivo) de cada control del formulario en el momento de la llamada, no un snapshot anterior; como ambas líneas se ejecutan de forma síncrona sin ningún `await` entre medio, `formData` capturaba el campo `message` ya vacío en cualquier envío de texto normal desde la UI real. Esto nunca se detectó en las fases anteriores porque, por instrucción explícita del usuario, ninguna fase probó el flujo de chat en un navegador real (todas las verificaciones fueron a nivel Python/Jinja/pytest, sin ejecutar la ruta HTTP real disparada por el JS del navegador). Fix: se reordenó `submitChat()` para construir `formData` **antes** de llamar a `renderOutgoingMessage(form)` (`app/templates/chat.html`), preservando que `resetAttachmentsUI()` siga ejecutándose después de capturar `formData` (comportamiento ya correcto desde Fase 9 para adjuntos, no tocado). Verificación empírica con DOM real (no solo lectura de spec): se instaló `jsdom` como dependencia de desarrollo (autorización explícita del usuario, ver más abajo) y se escribió `tests/js/test_submit_chat_formdata.mjs`, que carga el `<script>` real de `chat.html` (mismo mecanismo de extracción que `node --check`) en un DOM jsdom con el formulario real reproducido, stubea `fetch`/`DataTransfer` (jsdom no los implementa) y ejecuta la función `submitChat` real sin modificar. Prueba positiva (fix aplicado): `Valor real del campo 'message' en el FormData enviado: "Hola, este es un mensaje real escrito por el usuario"` → `PASS`. Prueba negativa (orden original revertido temporalmente en `chat.html`, para confirmar que el test detecta el bug y no es un falso positivo): `Valor real del campo 'message' en el FormData enviado: ""` → `FAIL: se esperaba message="Hola, este es un mensaje real escrito por el usuario" pero el FormData real contenia message=""`, confirmando que el bug era real. Archivo restaurado al estado con el fix inmediatamente después (`git diff --stat` verificado en 1 línea) y revalidado: `ruff check .` OK, `mypy app/` OK, `pytest -q` OK (120 passed), `node --check` OK, test jsdom nuevamente `PASS`.
- Hallazgo Fase 9/11 (discrepancia de `docs/ui-design.md`), confirmado y corregido: `docs/ui-design.md` §5, §6 y §11 documentaban `POST /api/chat` como la ruta de envío de chat con adjuntos; la UI real (`chat.html`) envía por `POST /api/chat/stream` vía `fetch` (SSE), no por HTMX. Se corrigieron las 3 secciones para reflejar que `POST /api/chat/stream` es la ruta real usada por la UI (sin HTMX, con eventos `tick`/`message_html`/`error`), y que `POST /api/chat` es una ruta equivalente sin streaming, con la misma cobertura de adjuntos/selector de modelo, mantenida por contrato pero no invocada por la UI. Se aplicó la misma corrección en `docs/technical-brief.md` §4 (fila nueva para `POST /api/chat/stream` en la tabla de rutas + nota aclaratoria).
- Bugs encontrados/corregidos: el de `chat.html` descrito arriba (preexistente de Fase 9, no de esta fase). Ningún bug nuevo introducido en el código propio de esta fase.
- Discrepancias de spec: sin discrepancias de spec (los 2 valores no fijados por el checklist fueron confirmados con el usuario antes de escribir código; el punto de los `type: ignore` y el de dónde documentar se resolvieron por investigación propia sin necesitar pregunta).
- Archivos nuevos: `tests/unit/test_config.py`, `package.json`, `package-lock.json` (dependencia de desarrollo `jsdom`, primer toolchain de Node persistido en el repo — hasta ahora solo se usaba `node --check` ad hoc, sin ningún archivo de proyecto Node), `tests/js/test_submit_chat_formdata.mjs`. Modificado además `.gitignore` (`node_modules/`).
- Archivos tocados: `app/main.py` (`@app.on_event("startup")` → `lifespan` con `asynccontextmanager`; `SessionMiddleware(..., https_only=settings.is_production)`), `app/config.py` (`environment`, `is_production`), `.env.example` (`ENVIRONMENT=development`), `app/routers/auth.py` (`secure=get_settings().is_production` en las 4 llamadas `set_cookie`), `app/db/queries/sessions.py` (4 campos nuevos en `AgentSession`), `app/templates/chat.html` (fix de orden en `submitChat`), `docs/technical-brief.md` (§4 tabla de rutas + nota `POST /api/chat/stream`, §11 variable `ENVIRONMENT`), `docs/ui-design.md` (§5, §6, §11), `tests/integration/test_auth_routes.py` (2 tests nuevos de `secure` condicional).
- Resultado de validaciones:
  - Final de fase: `ruff check .` OK, `mypy app/` OK (55 archivos), `pytest -q` OK (`120 passed`, +5 respecto del baseline).
  - Tests de aceptación: `tests/unit/test_config.py` (`is_production` falso por default, falso para valores no-`production`, verdadero solo para `production`); `tests/integration/test_auth_routes.py::test_login_cookies_not_secure_by_default` y `::test_login_cookies_secure_when_environment_is_production` (verifican el atributo `Secure` real en el header `Set-Cookie` de la respuesta HTTP). El resto de la suite existente (120 tests) confirma que la migración a `lifespan` no rompe el arranque de la app bajo `TestClient` (que ya ejecuta el lifespan real en cada test) ni ninguna ruta existente.
  - Verificación empírica adicional (fuera de `pytest`, requiere Node + `npm install` previo): `node tests/js/test_submit_chat_formdata.mjs` → `PASS` sobre el código con el fix aplicado; `FAIL` (con el mensaje llegando vacío) al revertir temporalmente el orden original, confirmando que el bug era real y que el test lo detecta.
- Fase NO marcada como HECHO por instrucción explícita del usuario; queda en estado `EN PROGRESO` en `docs/agent_total-plan.md` con el checklist tildado (incluyendo los 3 hallazgos acumulados de fases anteriores, resueltos dentro de esta fase). Sin commitear por instrucción implícita de continuidad con el procedimiento de fases anteriores (cierre pendiente de confirmación separada del usuario).
- Confirmación explícita del usuario en el chat de planificación sobre las 2 respuestas de alcance (ENVIRONMENT, columnas de agent_sessions) y sobre la instalación de jsdom como dependencia de desarrollo nueva para verificación empírica del bug crítico de chat.html; fase cierra sin discrepancias pendientes.

## Fase 15 - Consolidación de plantilla

- Fecha: 2026-07-05.
- Baseline de fase (previo a cambios de código/doc): `ruff check .` OK, `mypy app/` OK (55 archivos), `pytest -q` OK (`120 passed`), `node tests/js/test_submit_chat_formdata.mjs` OK (`PASS`) — coincide con el "Final de fase" reportado al cierre de Fase 14, sobre working tree limpio en el commit `88424a5`.
- Auditoría de estados (`grep -n "^Estado:" docs/agent_total-plan.md`): confirmado que las Fases 0 a 14 están en `HECHO`; Fase 15 es la única `PENDIENTE` (esta misma).
- Auditoría línea por línea de `docs/agent_total-changelog.md` completo (343 líneas) cruzando cada "Sesión dedicada de documentación" y cada "Discrepancias de spec" contra el contenido actual de `README.md`, `docs/technical-brief.md` y `docs/ui-design.md`:
  - Las 5 sesiones dedicadas conocidas (archivo de continuidad Fase 1, compactación §7 Fase 5, límites de runtime §5 Fase 6, `EVAL_USER_ID` §11 Fase 8, texto opcional con adjuntos §10.1/§6 Fase 9) están correctamente sincronizadas, sin faltantes.
  - Las discrepancias de Fase 9/11 sobre `docs/ui-design.md` (`/api/chat` vs `/api/chat/stream`) y el bug de `chat.html`, ambas asignadas a Fase 14, se verificaron efectivamente resueltas.
  - Hallazgo nuevo de esta auditoría (no reportado como discrepancia en su momento por ninguna fase anterior): `docs/ui-design.md` §5 (tabla de contrato HTMX) documentaba `hx-target: #session-item-{id}` para las filas "Archivar sesión"/"Eliminar sesión" — un selector que **no existe** en el markup real. La implementación real de Fase 13 (`app/templates/partials/session_item.html`) usa `hx-target="closest [data-session-item]"`. No era una omisión menor sino una afirmación incorrecta sobre el mecanismo real.
  - Segundo hallazgo nuevo: la sección `docs/technical-brief.md` "Sesiones: título automático, archivar y eliminar" seguía titulada "(plan Fase 13)" (lenguaje de plan pre-implementación) y le faltaban 3 decisiones confirmadas explícitamente por el usuario durante la implementación real de Fase 13 (ya registradas en la entrada de changelog de Fase 13, nunca elevadas a la documentación congelada): el texto exacto del `hx-confirm` de "Eliminar", el orden de operaciones del hard-delete (checkpointer primero, best-effort, con la razón de seguridad), y la definición operativa de "primer mensaje de usuario" (ignora turnos solo-adjuntos).
  - Ninguna de las dos fases (9 ni 13) había declarado estos dos puntos como "discrepancia de spec" en su momento; quedaron sin sincronizar hasta esta auditoría.
- Autorización: explícita del usuario, tras revisar el reporte de auditoría, para corregir directamente ambos hallazgos en esta misma fase (sin sesión dedicada aparte), tal como prevé el criterio de aceptación de Fase 15 ("esta es la única sesión donde se autoriza corregir documentación... porque es en sí misma la sesión dedicada de cierre").
- Corrección aplicada:
  - `docs/ui-design.md` §5: `hx-target` de las filas "Archivar sesión"/"Eliminar sesión" corregido de `#session-item-{id}` a `closest [data-session-item]` (coincide exactamente con `app/templates/partials/session_item.html`).
  - `docs/technical-brief.md`: sección renombrada a "Sesiones: título automático, archivar y eliminar" (sin "(plan Fase 13)"); agregadas las 3 decisiones faltantes (texto exacto del `hx-confirm`, orden checkpointer-primero con su razón de seguridad, definición operativa de "primer mensaje de usuario" ignorando solo-adjuntos) y, adicionalmente, el criterio de reintentos sin límite (implícito antes, ahora explícito) y la aclaración de que la migración 00007 ya está aplicada contra Supabase real (antes decía "definidos para", ahora "aplicados vía").
- Hallazgos de staleness identificados pero **no corregidos todavía** en este paso (quedan para el resumen "as-built" de esta misma fase, pendiente de definir con el usuario): `README.md` ("Estado actual del repositorio" y "Pendientes reales de esta etapa") y `docs/technical-brief.md` §2 ("Pendiente o incompleto hoy") y §12 ("Estado operativo y próximos pasos") describen el estado del proyecto anterior a la Fase 4 (memoria, compactación etapa 2, Langfuse, evaluaciones, adjuntos/selector de modelo, MCP figuran como "pendientes" pese a estar `HECHO` desde hace varias fases). No es un hallazgo puntual de una decisión perdida sino la consolidación final que corresponde al resumen as-built.
- Bugs encontrados/corregidos: ninguno de código; los 2 hallazgos de esta fase son de documentación (ver arriba).
- Discrepancias de spec: las 2 descritas arriba, corregidas en esta misma sesión.
- Fase NO marcada como HECHO todavía; el resumen "as-built" y la revisión de las secciones de staleness (README.md, technical-brief.md §2/§12) quedan pendientes de definir con el usuario antes del cierre. No commiteado.
- Resumen as-built producido en archivo nuevo dedicado `docs/agent_total-as-built.md` (confirmado por el usuario: no como sección de `README.md`), con las 4 secciones acordadas (qué se construyó, qué quedó fuera de alcance a propósito, lección aprendida del bug crítico de `chat.html` con la redacción provista por el usuario, y referencia al changelog completo). Agregada fila en la tabla "Fuentes de verdad" de `README.md` apuntando a este archivo nuevo, para que sea descubrible.
- Staleness corregido (no es narrativa nueva, es limpieza de hechos, autorizada explícitamente por el usuario en esta misma sesión): `README.md` ("Estado actual del repositorio" y "Pendientes reales de esta etapa") y `docs/technical-brief.md` §2 ("Implementado en código hoy" / sección renombrada de "Pendiente o incompleto hoy" a "Fuera de alcance a propósito") y §12 ("Estado operativo y próximos pasos") actualizados para reflejar que memoria, compactación etapa 2, Langfuse, evaluaciones, adjuntos/selector de modelo, MCP, gestión de sesiones y hardening de cierre están `HECHO`, distinguiendo explícitamente lo "fuera de alcance a propósito" (documentado en `docs/agent_total-as-built.md`) de lo genuinamente pendiente (no queda nada bloqueante).
- Resultado final de validaciones (Paso 4, tras el resumen as-built y la corrección de staleness): `ruff check .` OK, `mypy app/` OK (55 archivos), `pytest -q` OK (`120 passed`), `node tests/js/test_submit_chat_formdata.mjs` OK (`PASS`) — sin regresiones respecto del baseline de esta fase.

## Sesión dedicada de corrección - flush_session_memory persistía respuesta del asistente (§8)

- Fecha: 2026-07-05.
- Motivo que activó la sesión: al validar el entorno local recién levantado y probar memoria de largo plazo (Fase 4) contra Supabase real, se encontró que `flush_session_memory()` en `app/agent/memory_flush.py` persistía como "memoria episódica" el ÚLTIMO MENSAJE DE LA SESIÓN sin filtrar por rol (`messages[-1]`), que en el flujo real de `app/routers/chat.py` siempre es la respuesta del ASISTENTE (`memory_flush` se agenda después de persistir esa respuesta) — no el mensaje del usuario, que es la intención real de una memoria de largo plazo pensada para capturar lo que el usuario comunica.
- Evidencia: se consultó la tabla `memories` real en Supabase para el usuario de prueba y las 10 filas encontradas contenían contenido de tipo respuesta-del-asistente (ej. "El texto es: meet.google.com.", "La imagen adjunta es el icono de la aplicación 'Flickr'."), confirmando el bug en producción, no solo en teoría.
- Corrección aplicada: `flush_session_memory` ahora busca el último mensaje con `role == "user"` en la lista devuelta por `get_session_messages` (en vez de tomar el último mensaje sin filtrar), y usa su `content`. Si no hay ningún mensaje de usuario, mantiene el mismo guard de return temprano.
- Verificación: se agregó test unitario nuevo (`test_flush_session_memory_persists_user_content_not_assistant`) usando instancias reales de `AgentMessage` para ejercer la lógica de selección por rol. Suite completa: 121 tests passed (120 previos + 1 nuevo), `ruff check .` limpio, `mypy app/` sin issues (55 archivos).
- Hallazgo adicional NO corregido en esta sesión (dejar registrado para atención futura): 2 tests preexistentes en `tests/integration/test_memory_pipeline.py` pasan porque usan dicts planos en vez de objetos con atributos `.content`/`.role`, lo cual causa un `AttributeError` silenciado por el `try/except` genérico de `flush_session_memory` — dan verde sin ejercer la lógica real que dicen cubrir. No se corrigió porque está fuera del alcance de este fix puntual.
- Autorización: explícita del usuario, mismo procedimiento que sesiones dedicadas anteriores (Fase 8, Fase 13).
- Archivo(s) cambiado(s): `app/agent/memory_flush.py` (lógica de selección de mensaje), `tests/integration/test_memory_pipeline.py` (test nuevo agregado).
- Los 7 archivos modificados en esta sesión se agruparon en 4 commits atómicos con prefijo `fix:`: callback de Langfuse (`app/agent/langfuse.py`), `max_tokens`/timeout del modelo (`app/agent/model.py`), dependencias `psycopg[binary]`+`langchain` (`pyproject.toml`, `uv.lock`), y este fix de memoria episódica junto a su test y este mismo changelog (`app/agent/memory_flush.py`, `tests/integration/test_memory_pipeline.py`, `docs/agent_total-changelog.md`).

## Sesión dedicada - Clasificación automática de memoria episodic/semantic

- Fecha: 2026-07-05.
- Motivo: extensión del alcance de memoria de largo plazo, más allá del fix de rol previo (§8, sesión anterior de este mismo día). El schema de `memories` (`migrations/00004_long_term_memory.sql`) siempre permitió 3 tipos (`episodic`, `semantic`, `procedural`), pero solo `episodic` tenía productor real: `flush_session_memory()` guardaba todo turno con el literal hardcodeado `memory_type="episodic"`. Se implementó clasificación automática `episodic`/`semantic` para que el tipo persistido refleje la naturaleza real del contenido.
- Diseño: nuevo `app/agent/memory_classifier.py` con `classify_memory_type(content: str) -> str`, siguiendo exactamente el mismo patrón que `app/agent/session_title.py` (llamada liviana vía `create_compaction_model()` sin argumentos, mensajes `[SystemMessage, HumanMessage]`, parseo de la respuesta con `strip().lower()`, y el mismo manejo de errores: todo el cuerpo en un único `try/except Exception as exc` con `logger.warning(...)` y `# pragma: no cover - external services`). Fallback seguro a `"episodic"` tanto ante una respuesta ambigua del modelo (cualquier valor que no sea exactamente `"episodic"` o `"semantic"`) como ante cualquier excepción — nunca debe romper el flush. Se invoca en `flush_session_memory()` (`app/agent/memory_flush.py`) inmediatamente después de `can_store_memory(latest)` y antes de `generate_embedding(latest)`, para no gastar la llamada al LLM clasificador en contenido que de todas formas se descarta por el filtro de privacidad.
- Recuperación: `memory_injection_node` (`app/agent/nodes/memory_injection_node.py`) ahora lee el campo `type` de cada memoria devuelta por `match_memories()` (antes se descartaba sin usar) y agrupa el bloque inyectado en el prompt en 2 secciones: las memorias `semantic` bajo un header nuevo `[HECHOS Y PREFERENCIAS DEL USUARIO]`, y las `episodic` (o sin `type`, por compatibilidad con datos preexistentes) bajo el header ya existente `[MEMORIA DEL USUARIO]`, sin cambiarlo. Cualquier sección que quede vacía se omite completa (no se imprime un header sin contenido debajo). Firma de `memory_injection_node`, invocación de `match_memories()` e `increment_memory_retrieval_count()` sin cambios.
- Alcance: `procedural` queda deliberadamente sin implementar — mismo patrón que el stub de MCP (Fase 12): valor reservado en el schema, sin productor real construido.
- Verificación (suite automatizada): `130 passed` (127 previos + 3 nuevos: `tests/unit/test_memory_classifier.py` con 4 casos — respuesta `semantic` limpia, `episodic` limpia, ambigua→default `episodic`, excepción del modelo→default `episodic`, nunca propaga —; `tests/unit/test_memory_flush.py` con test de integración que mockea `classify_memory_type` y confirma que `save_memory` recibe el `memory_type` clasificado en vez del literal hardcodeado, más un test que confirma que la clasificación NO se invoca cuando `can_store_memory` filtra el contenido; y 3 tests nuevos en `tests/unit/test_memory_injection.py` para `_format_memory_block` con mix de tipos, sección `semantic` vacía omitida, sección `episodic` vacía omitida — los 2 tests preexistentes de ese archivo se ajustaron para incluir `"type": "episodic"` explícito en sus fixtures, de forma que sigan pasando por la razón correcta y no por casualidad). `ruff check .` y `mypy app/` limpios (56 archivos).
- Verificación end-to-end contra Supabase real (no solo mocks): con sesión de navegador real y cookie `sb-access-token` vigente, se creó una sesión nueva y se envió el mensaje `"Vivo en Cali, Colombia y estudié ingeniería."`, persistido en `memories` con `type = "semantic"` (confirmado por consulta directa a la tabla vía `psycopg`, contenido idéntico al mensaje del usuario). En la misma sesión, el mensaje `"Se me acaba de trabar el navegador probando esto."` se persistió con `type = "episodic"`. En una sesión nueva distinta, la pregunta `"¿Dónde vivo y qué estudié?"` fue respondida correctamente por el asistente ("Vives en Cali, Colombia, y estudiaste ingeniería."), confirmando que `memory_injection_node` recuperó y agrupó la memoria `semantic` bajo su header nuevo. Consulta posterior a `memories` confirmó `retrieval_count`/`last_retrieved_at` actualizados selectivamente solo en la fila `semantic` efectivamente usada para responder (de `retrieval_count=0`/`last_retrieved_at=None` a `retrieval_count=1`/timestamp real), mientras que la fila `episodic` de esa misma sesión permaneció en `retrieval_count=0` por no ser relevante a la pregunta.
- Documentación actualizada en el mismo trabajo (autorizada explícitamente por el usuario junto con el resto de esta sesión, no una sesión dedicada de documentación aparte): `docs/technical-brief.md` §8 ("Alcance real de tipos de memoria") y `docs/agent_total-as-built.md` ("Qué quedó fuera de alcance a propósito"), ambos corregidos para reflejar que `episodic` y `semantic` tienen productor real vía `classify_memory_type()`, y que solo `procedural` queda fuera de alcance.
- Bugs encontrados/corregidos: ninguno preexistente en el código tocado.
- Discrepancias de spec: ninguna.
- Autorización: explícita del usuario, mismo procedimiento que sesiones dedicadas anteriores (Fase 8, Fase 13, sesión de corrección de rol de memoria episódica).
- Archivo(s) cambiado(s): `app/agent/memory_classifier.py` (nuevo), `app/agent/memory_flush.py`, `app/agent/nodes/memory_injection_node.py`, `tests/unit/test_memory_classifier.py` (nuevo), `tests/unit/test_memory_flush.py` (nuevo), `tests/unit/test_memory_injection.py`, `docs/technical-brief.md`, `docs/agent_total-as-built.md`.

## Sesión dedicada - Tercer tipo de memoria: procedural

- Fecha: 2026-07-05.
- Motivo: completar el alcance de tipos de memoria. En la sesión previa de este mismo día ya se habían implementado `episodic` y `semantic` (`classify_memory_type()`); esta sesión agrega `procedural` como tercer tipo, cerrando los 3 valores que el schema de `memories` (`migrations/00004_long_term_memory.sql`) siempre permitió.
- Definición usada para `procedural` (decisión explícita del usuario, ambas cosas en una sola categoría): preferencias de estilo de interacción con el asistente (formato, tono, workflow) Y/O procedimientos o pasos concretos que el usuario enseña o pide recordar (una receta, un proceso técnico propio).
- Cambios: `CLASSIFIER_SYSTEM_PROMPT` en `app/agent/memory_classifier.py` extendido de 2 a 3 categorías, con la definición de `procedural` descrita arriba; el check de validación de la respuesta ahora es `label not in ("episodic", "semantic", "procedural")`. El fallback ante ambigüedad o excepción del modelo sigue siendo `"episodic"` — nunca `"procedural"` — mismo criterio de seguridad que la implementación de 2 tipos. `memory_injection_node` (`app/agent/nodes/memory_injection_node.py`) agrega el header nuevo `PROCEDURAL_HEADER = "[FORMA DE TRABAJO Y PROCEDIMIENTOS DEL USUARIO]"` y reestructura `_format_memory_block()` en 3 buckets explícitos (`semantic`, `procedural`, y todo lo demás incluyendo `episodic`/tipo desconocido/tipo faltante, que cae en el bucket episodic por el mismo principio de robustez que el fallback del clasificador), ensamblados en ese orden — semantic → procedural → episodic, de lo más estable a lo más transitorio — con cada sección omitida si queda vacía. Firma del nodo y llamadas a `match_memories()`/`increment_memory_retrieval_count()` sin cambios.
- Verificación (suite automatizada): `132 passed` (130 previos + 2 nuevos: `test_classify_memory_type_returns_procedural` en `tests/unit/test_memory_classifier.py`, y `test_format_memory_block_omits_empty_procedural_section` en `tests/unit/test_memory_injection.py`; el test existente `test_format_memory_block_separates_sections_by_type` se reescribió para cubrir los 3 tipos simultáneamente más un valor de `type` desconocido y uno faltante, confirmando el orden de las 3 secciones y que ambos casos caen en el bucket episodic). `ruff check .` y `mypy app/` limpios (56 archivos).
- Verificación end-to-end contra Supabase real: con sesión de navegador real (token verificado vigente por `exp` del JWT antes de usarlo), se creó una sesión nueva y se envió el mensaje `"De ahora en adelante, cuando me expliques algo técnico, respondeme siempre con listas numeradas cortas, sin párrafos largos."`, persistido en `memories` con `type = "procedural"` (confirmado por consulta directa vía `psycopg`). En la misma sesión, un mensaje de control (`"Mi comida favorita es el sushi."`) se persistió con `type = "semantic"`, confirmando que las 3 categorías conviven sin interferirse en una misma sesión. En una sesión nueva distinta, la pregunta `"Explicame cómo funciona la compactación de contexto en este proyecto."` fue respondida por el asistente en formato de lista numerada corta (4 ítems, sin párrafos largos) en vez del párrafo explicativo habitual — evidencia de **cambio de comportamiento** inducido por la memoria procedural inyectada por `memory_injection_node`, no solo de recuperación de contenido textual (a diferencia de la validación de `semantic` de la sesión previa, donde la evidencia era que el asistente mencionara un dato). Consulta posterior a `memories` confirmó `retrieval_count`/`last_retrieved_at` de la fila procedural actualizados de `0`/`None` a `1`/timestamp real, exactamente tras el turno que la usó.
- Documentación actualizada en el mismo trabajo: `docs/technical-brief.md` §8 y `docs/agent_total-as-built.md`, reflejando que los 3 tipos del schema (`migrations/00004`) ahora tienen productor real vía `classify_memory_type()` — ya no queda ningún tipo de memoria "fuera de alcance".
- Bugs encontrados/corregidos: ninguno preexistente en el código tocado.
- Discrepancias de spec: ninguna.
...
## Sesión dedicada de documentación - eliminación completa del canal Telegram

- Fecha: 2026-07-05.
- Motivo que activó la sesión: el usuario cuestionó por qué seguían existiendo referencias y migraciones a Telegram si es un canal que nunca tuvo código de aplicación real en `agent_total` (ya eliminado en Fase 3: `app/routers/telegram.py`, `app/services/telegram_bot.py`, `app/db/queries/telegram.py`). La decisión previa (Fase 1) había sido mantener `telegram_accounts`/`telegram_link_codes` y el valor `'telegram'` del `CHECK` de `agent_sessions.channel` como reservado para un posible regreso futuro, documentando explícitamente que cualquier `DROP` requería autorización separada. El usuario dio esa autorización explícita en esta sesión.
- Autorización: explícita del usuario, en dos pasos separados de esta misma sesión — (1) aplicar una migración nueva (`00008_remove_telegram_scope.sql`) contra Supabase real para dropear `telegram_accounts`/`telegram_link_codes` y ajustar el `CHECK` de `channel`; (2) tras confirmar que ambas tablas tenían 0 filas y que `agent_sessions.channel` solo tenía valores `'web'` en producción, reescribir retroactivamente `migrations/00001_initial_schema.sql` y `migrations/00003_scheduled_tasks.sql` para que nunca hubieran incluido Telegram, y eliminar `00008` por quedar redundante.
- Nota de gobernanza: `.cursor/.rules/change-control.mdc` declara las migraciones ya mergeadas como inmutables ("toda migración ya creada queda inmutable una vez mergeada"). Se editaron `00001` y `00003` de todas formas, por instrucción explícita y repetida del usuario dentro de esta sesión dedicada — que es precisamente el mecanismo que el propio `change-control.mdc` exige para excepciones ("un pedido explícito y dedicado del usuario, separado de un pedido de implementación de fase"). Este archivo de changelog no se edita retroactivamente (regla propia, ver encabezado): esta entrada se agrega al final para dejar registro de la excepción, sin alterar ninguna entrada anterior.
- Cambios aplicados:
  - Migración nueva `migrations/00008_remove_telegram_scope.sql` aplicada contra Supabase real (verificado antes: `telegram_accounts` y `telegram_link_codes` con 0 filas; `agent_sessions.channel` solo con valores `'web'`) y luego eliminada del repo por quedar redundante tras el paso siguiente.
  - `migrations/00001_initial_schema.sql`: eliminados los bloques `telegram_accounts` y `telegram_link_codes` (tablas, RLS, policies); `channel` pasa de `check (channel in ('web', 'telegram'))` a `check (channel in ('web'))`.
  - `migrations/00003_scheduled_tasks.sql`: `CHECK (channel IN ('web', 'telegram', 'cron'))` → `CHECK (channel IN ('web', 'cron'))`.
  - `docs/agent_total-plan.md`: Fase 1 ("Mantener intactas...") y Fase 3 (bullets de archivos eliminados y criterio de aceptación) sin menciones a Telegram.
  - `.cursor/.rules/architecture.mdc`: línea de alcance de canales y ejemplo de código sin menciones a Telegram (el ejemplo ahora usa `cron`, que sigue siendo un canal reservado real).
- Verificación posterior: `psql`/`psycopg` contra Supabase real confirmó cero tablas `telegram_*` y el `CHECK` de `channel` en `('web', 'cron')`; `uv run pytest -q` → `173 passed` sin regresiones (la app ya tenía cero referencias a Telegram desde Fase 3, así que ningún test dependía de estas tablas).
- Bugs encontrados/corregidos: ninguno.
- Discrepancias de spec: ninguna — este cambio revierte una decisión de alcance anterior (Fase 1: mantener Telegram reservado) por autorización explícita posterior del usuario, no corrige un error de implementación.
- Archivo(s) cambiado(s): `migrations/00001_initial_schema.sql`, `migrations/00003_scheduled_tasks.sql` (editados retroactivamente, ver nota de gobernanza arriba), `migrations/00008_remove_telegram_scope.sql` (creado y luego eliminado), `docs/agent_total-plan.md`, `.cursor/.rules/architecture.mdc`, `docs/agent_total-changelog.md` (esta entrada).
